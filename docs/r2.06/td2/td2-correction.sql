...
SELECT idV, villeDep, paysArr, numCl, ville
FROM Voyage
LEFT OUTER JOIN Client ON villeDep = ville AND numCl > 2200;
...
WHERE (CaG.prix = 0 OR CaG.prix IS NULL)
...
SELECT categorie
FROM T
WHERE nbClients >= ALL (SELECT nbClients
						FROM T);
...
WHERE (C2.prix = 0 OR C2.prix IS NULL)
  AND (C1.prix <> 0 OR C1.prix IS NOT NULL);

-- Version alternative.
PROMPT "Q25 - V2";

WITH T (code, libelle, prix) AS (
...
WITH T (numCl, nom, prenom, idV) AS (
...
FROM T
WHERE (nom <> 'PEYROCHE' OR  prenom <> 'ARNAUD')
...
WITH T (numCl, nom, prenom, nbPers) AS (
...
FROM T
WHERE COALESCE(nbPers, 0) = (SELECT MAX(COALESCE(nbPers, 0))
                             FROM T);
...
FROM Voyage
WHERE paysArr NOT IN (SELECT paysArr
                      FROM Voyage
                      WHERE nbEtoiles = (SELECT MAX(nbEtoiles)
                                         FROM Voyage));
