...
-- Version imbriquée.
-- Remarque : la version imbriquée ne peut pas être effectuée utilement car dateDep et tarif sont des attributs de Planning et villeArr est un attribut de Voyage.
...
WHERE code IN (SELECT code
               ...
               WHERE (prix = 0 OR prix IS NULL)
      		     AND idV IN (SELECT idV FROM Reservation
...
INNER JOIN Voyage V ON R.idV = V.idV AND villeDep = ville
...
FROM Client
WHERE (numCl, ville) IN (SELECT numCl, villeDep
						 FROM Reservation R
						 INNER JOIN Voyage V ON R.idV = V.idV
						 AND villeArr = 'ISTANBUL');

-- Remarque : la version imbriquée ne peut pas être effectuée parfaitement car numCl est un attribut de Reservation et villeDep un attribut de Voyage.
...
FROM Voyage V
INNER JOIN Carac C1 ON V.idV = C1.idV
INNER JOIN OptionV O1 ON C1.code = O1.code
INNER JOIN Carac C2 ON V.idV = C2.idV
INNER JOIN OptionV O2 ON C2.code = O2.code
...
-- Remarque : les commandes précédentes sont nécessaires si les altérations de table n'ont pas été faites précédemment. Elles ne doivent pas être exécutées dans le cas contraire.
...
-- Remarque : la commande précédente est nécessaire si l'altération de table n'a pas été faite précédemment. Elle ne doit pas être exécutée dans le cas contraire.
...
-- Remarque : les commandes précédentes sont nécessaires si la création de table n'a pas été faite précédemment. Il est inutile de les exécuter dans le cas contraire.
...
-- Remarque : les commandes précédentes sont nécessaires si l'alteration et la création de table n'ont pas été faites précédemment. L'altération ne doit pas être exécutée, et il est inutile d'exécuter la création dans le cas contraire.
